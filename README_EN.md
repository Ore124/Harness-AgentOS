...
This project (Harness Agent + MiniMax M2.7 Highspeed) achieved a ranking on the [Terminal-Bench 2.0](https://www.tbench.ai/leaderboard/terminal-bench/2.0) leaderboard:
...
| Submitted by | lazyFrogLOL |
| Date | 2026-05-14 |

> 🔗 [View detailed task scores](https://www.tbench.ai/leaderboard/terminal-bench/2.0/harness-agent/unknown/MiniMax-M2.7-highspeed%40minimax)

**Highlight tasks (100% pass rate):**
cobol-modernization, code-from-image, constraints-scheduling, configure-git-webserver, custom-memory-heap-crash, extract-elf, fix-code-vulnerability, fix-git, git-leak-recovery, git-multibranch, headless-terminal, hf-model-inference, kv-store-grpc, log-summary-date-ranges, modernize-scientific-stack, multi-source-data-merger, nginx-request-logging, openssl-selfsigned-cert, pypi-server, reshard-c4-data, vulnerable-secret

> Note: This is an educational reproduction project using pure Python with no Agent SDK dependencies. Among 143 entries (including submissions from top teams like OpenAI, Anthropic, Google, JetBrains, etc.), this result validates the effectiveness of the architectural design described in the article.

📖 [View full optimization log](docs/OPTIMIZATION_LOG_EN.md) — The complete journey from 0% to 43.8%, with detailed reasoning behind every key decision.
...
The project supports multiple task scenarios (switchable via the Profile system):

| Profile | Use Case | Description |
|---------|----------|-------------|
| `app-builder` | Web app building | Original article scenario, with Playwright browser testing |
| `terminal` | Terminal tasks | Adapted for Terminal-Bench 2.0, with 35 task-specific Skills |
| `swe-bench` | Code repair | SWE-Bench style bug fixing |
| `reasoning` | Pure reasoning | Q&A / math / logic reasoning |

Key features:
- **Middleware system**: loop detection, pre-exit verification, time budget, skeleton code detection, error guidance
- **Profile system**: different task scenarios use different prompt / tool / middleware combinations
- **Skill progressive disclosure**: 35 task-specific Skills, agent autonomously decides when to load
- **Harbor integration**: one-click Terminal-Bench 2.0 benchmark runs
...
### Running Terminal-Bench 2.0 Benchmark
...
# Full benchmark (5 trials/task, required for leaderboard)
./run_benchmark_v2.sh
```
...
├── middlewares.py          # Agent behavior middlewares (loop detection / pre-exit verification /
│                             time budget / task tracking / skeleton detection / error guidance)
...
│   ├── app_builder.py      #   Web app building (original article scenario)
│   ├── terminal.py         #   Terminal tasks (Terminal-Bench 2.0 adaptation)
│   ├── swe_bench.py        #   SWE-Bench code repair
│   └── reasoning.py        #   Pure reasoning Q&A
│
├── benchmarks/             # Terminal-Bench 2.0 benchmark integration
│   ├── harbor_agent.py     #   Harbor framework adapter (in-container install + run)
│   ├── tb2_tasks.json      #   TB2 task metadata (timeouts / categories)
│   └── README.md           #   Benchmark usage instructions
│
├── skills/                 # Skill directory (35 task-specific skills)
│   ├── frontend-design/    #   Frontend design guidance
│   ├── chess-best-move/    #   Chess best move
│   ├── qemu-startup/       #   QEMU VM startup
│   ├── crack-7z-hash/      #   7z hash cracking
│   ├── ...                 #   (35 total, covering most TB2 tasks)
│   └── vulnerable-secret/  #   Security vulnerability detection
│
├── scripts/                # Utility scripts
│   └── analyze_results.py  #   Analyze TB2 run results (pass rate / failure classification / retry cmd)
│
├── vendor_wheels/          # Offline Python packages (for containers without network)
│                             openai, pydantic, httpx, anyio, etc.
│
├── .github/workflows/      # CI/CD
│   └── terminal-bench.yml  #   GitHub Actions for automated TB2 benchmark runs
│
├── run_benchmark_v2.sh     # One-click local TB2 benchmark runner
...
COMPRESS_THRESHOLD=50000       # token count to trigger compaction
...
Each profile supports environment variable overrides with format `PROFILE_<NAME>_<KEY>`:

```bash
# Override terminal profile pass threshold
PROFILE_TERMINAL_PASS_THRESHOLD=9.0

# Override terminal profile max rounds
...
```bash
# From the original article (default app-builder profile)
...
Currently includes 35 Skills covering most Terminal-Bench 2.0 task types (cryptography, compilers, VMs, data processing, security, etc.).

## Dependencies

- Python 3.10+
- `openai` — API client (compatible with any OpenAI-format endpoint)
- `tiktoken` (optional) — accurate token counting, falls back to character estimation if missing
- `playwright` (optional) — browser testing, only needed for app-builder profile
- `harbor` (optional) — for running Terminal-Bench 2.0 benchmarks
