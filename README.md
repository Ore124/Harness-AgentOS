...
## 🏆 Terminal-Bench 2.0 排行榜成绩

本项目（Harness Agent + MiniMax M2.7 Highspeed）在 [Terminal-Bench 2.0](https://www.tbench.ai/leaderboard/terminal-bench/2.0) 排行榜中取得了成绩：
...
> 🔗 [查看详细任务得分](https://www.tbench.ai/leaderboard/terminal-bench/2.0/harness-agent/unknown/MiniMax-M2.7-highspeed%40minimax)
...
> 注：本项目是一个教学复现项目，使用纯 Python 实现，不依赖任何 Agent SDK。在 143 个参赛条目中（包含来自 OpenAI、Anthropic、Google、JetBrains 等顶级团队的提交），取得此成绩验证了文章中架构设计的有效性。

📖 [查看完整优化记录](docs/OPTIMIZATION_LOG.md) — 从 0% 到 43.8% 的详细优化历程，包含每个关键决策的原因和效果。
...
项目支持多种任务场景（通过 Profile 系统切换）：

| Profile | 用途 | 说明 |
|---------|------|------|
| `app-builder` | Web 应用构建 | 文章原始场景，含 Playwright 浏览器测试 |
| `terminal` | 终端任务 | 适配 Terminal-Bench 2.0，含 35 个任务专用 Skill |
| `swe-bench` | 代码修复 | SWE-Bench 风格的 bug 修复 |
| `reasoning` | 纯推理 | 问答 / 数学 / 逻辑推理 |

核心特性：
- **中间件系统**：循环检测、退出验证、时间预算、骨架代码检测、错误引导
- **Profile 系统**：不同任务场景使用不同的 prompt / 工具集 / 中间件组合
- **Skill 渐进式披露**：35 个任务专用 Skill，Agent 自主决定何时加载
- **Harbor 集成**：一键运行 Terminal-Bench 2.0 基准测试
...
# 使用 terminal profile（Terminal-Bench 风格任务）
...
├── middlewares.py          # Agent 行为中间件（循环检测 / 退出验证 / 时间预算 /
│                             任务追踪 / 骨架检测 / 错误引导）
├── logger.py               # 彩色日志（Agent 颜色编码 + emoji 标记 + 阶段横幅）
├── skills.py               # Skill 注册表（渐进式披露 Level 1）
├── config.py               # 配置（自动加载 .env）
│
├── profiles/               # 任务场景 Profile 系统
│   ├── base.py             #   抽象基类 BaseProfile + AgentConfig + ProfileConfig
│   ├── app_builder.py      #   Web 应用构建（文章原始场景）
│   ├── terminal.py         #   终端任务（Terminal-Bench 2.0 适配）
│   ├── swe_bench.py        #   SWE-Bench 代码修复
│   └── reasoning.py        #   纯推理问答
│
├── benchmarks/             # Terminal-Bench 2.0 基准测试
│   ├── harbor_agent.py     #   Harbor 框架适配器（容器内安装 + 运行）
│   ├── tb2_tasks.json      #   TB2 任务元数据（超时 / 分类）
│   └── README.md           #   基准测试使用说明
│
├── skills/                 # Skill 目录（35 个任务专用 skill）
│   ├── frontend-design/    #   前端设计指导
│   ├── chess-best-move/    #   国际象棋最佳走法
│   ├── qemu-startup/       #   QEMU 虚拟机启动
│   ├── crack-7z-hash/      #   7z 哈希破解
│   ├── ...                 #   （共 35 个，覆盖 TB2 大部分任务）
│   └── vulnerable-secret/  #   安全漏洞检测
│
├── scripts/                # 辅助脚本
│   └── analyze_results.py  #   分析 TB2 运行结果（通过率 / 失败分类 / 重试命令）
│
├── vendor_wheels/          # 离线 Python 包（容器内无网络时使用）
│                             openai, pydantic, httpx, anyio, etc.
│
├── .github/workflows/      # CI/CD
│   └── terminal-bench.yml  #   GitHub Actions 自动运行 TB2 基准测试
│
├── run_benchmark_v2.sh     # 本地一键运行 TB2 基准测试脚本
├── .env.template           # 环境变量模板
...
COMPRESS_THRESHOLD=50000       # 触发上下文压缩的 token 数
RESET_THRESHOLD=100000         # 触发上下文重置的 token 数
MAX_AGENT_ITERATIONS=500       # 每个 Agent 最大工具调用轮数（实际由 TimeBudgetMiddleware 控制）
ENABLE_PARALLEL_TOOL_CALLS=0   # 并行工具调用（仅 Claude/GPT-4o 等支持的模型启用）
```

### Profile 级别配置

每个 Profile 支持通过环境变量覆盖参数，格式为 `PROFILE_<NAME>_<KEY>`：

```bash
# 覆盖 terminal profile 的通过阈值
PROFILE_TERMINAL_PASS_THRESHOLD=9.0

# 覆盖 terminal profile 的最大轮数
...
```bash
# 文章原文的两个案例（默认 app-builder profile）
...
当前已有 35 个 Skill，覆盖 Terminal-Bench 2.0 的大部分任务类型（密码学、编译器、虚拟机、数据处理、安全等）。

## 依赖

- Python 3.10+
- `openai` — API 客户端（兼容任何 OpenAI 格式端点）
- `tiktoken`（可选）— token 精确计数，缺失时回退到字符估算
- `playwright`（可选）— 浏览器测试，仅 app-builder profile 需要
- `harbor`（可选）— 运行 Terminal-Bench 2.0 基准测试
