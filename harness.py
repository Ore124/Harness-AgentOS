...
def main():
    ...
    # --resume <run_id|state_path|workspace_dir>
    resume_target = None
    if "--resume" in args:
        idx = args.index("--resume")
        if idx + 1 < len(args):
            resume_target = args[idx + 1]
            args = args[:idx] + args[idx + 2:]
        else:
            print("Error: --resume requires a run id, state file, or workspace directory")
            sys.exit(1)
    ...
    if resume_target:
        state_path = _resolve_resume_state_path(resume_target)
    ...
    if len(args) < 1:
        ...
        print("       python harness.py --resume <run_id|state_path|workspace_dir>")
        print("       python harness.py --ui [--port 8765]")
        print()
        print("Profiles:")
        for p in list_profiles():
            print(f"  {p['name']:15s} {p['description']}")
...
def _resolve_resume_state_path(target: str) -> Path:
    from orchestrator.state import STATE_FILE
    ...
    if p.exists():
        if p.is_dir():
            return p / STATE_FILE
        return p
    return Path(config.WORKSPACE).resolve() / target / STATE_FILE
